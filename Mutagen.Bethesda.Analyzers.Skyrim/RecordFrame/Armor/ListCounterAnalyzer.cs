...
namespace Mutagen.Bethesda.Analyzers.Skyrim.RecordFrame.Armor;
...
public class ListCounterAnalyzer : IIsolatedRecordFrameAnalyzer<IArmorGetter>
{
    ...
    public void AnalyzeRecord(IsolatedRecordFrameAnalyzerParams<IArmorGetter> param)
    {
        if (!param.Frame.TryFindSubrecord(RecordTypes.KSIZ, out var ksizFrame)) return;
        if (!param.Frame.TryFindSubrecord(RecordTypes.KWDA, out var kwdaFrame)) return;
        var count = ksizFrame.AsUInt32();
        var actualCount = kwdaFrame.ContentLength / 4;
        if (count == actualCount) return;
        param.AddTopic(
